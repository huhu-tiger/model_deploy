...
check_local_patches() {
  log "检查本机 vllm-patches..."
  [[ -f "${PATCH_DIR}/${PATCH_SITECUSTOMIZE}" ]] \
    || die "缺少 ${PATCH_DIR}/${PATCH_SITECUSTOMIZE}"
  [[ -f "${PATCH_DIR}/${PATCH_LONG_CTX}" ]] \
    || die "缺少 ${PATCH_DIR}/${PATCH_LONG_CTX}"
  [[ -f "${PATCH_DIR}/${PATCH_PTH}" ]] \
    || die "缺少 ${PATCH_DIR}/${PATCH_PTH}"
}
...
check_remote_compose_config() {
  ...
  local remote_tmp="/tmp/deepseek-v4-flash-vllm-compose-check-$$.yml"
  local remote_env="/tmp/deepseek-v4-flash-vllm-cluster-check-$$.env"
