#!/usr/bin/env bash
# MiniMax-M3-AWQ 双节点入口（本机 master + 远程 worker）
...
usage() {
  cat <<EOF
MiniMax-M3-AWQ 双节点部署（配置: cluster.env，推荐 make 入口）
...
用法:
  ...
  ./run start           # 启动双节点
  ./run stop            # 停止双节点
  ./run check           # 环境检测（含 hosts / ufw / IB / 镜像）
  ./run hosts           # 仅配置 /etc/hosts
  ./run firewall        # 仅配置 ufw 集群互通
  ./run status          # 容器状态
  ./run logs [svc]      # 本机日志（默认 minimax-m3-awq-int4-vllm）
  ./run master          # 仅本机 master
  ./run sync            # scp worker compose + cluster.env + parser
  ./run worker          # 仅远程 worker

首次部署:
  make init             # 复制 cluster.env.example → cluster.env
  编辑 cluster.env      # 填写 MASTER_IP / WORKER_IP
  两端准备镜像          # 在 ../minimax-M3-AWQ make build 后 docker save/load 或 pull
  make config-check
  make restart
