...
check_local_image() {
  ...
  docker image inspect "${DOCKER_IMAGE}" >/dev/null 2>&1 \
    || die "本机缺少镜像 ${DOCKER_IMAGE}（请先在 ../minimax-M3-AWQ make build，或 docker pull）"
}
...
check_parser_file() {
  [[ -f "${PARSER_FILE}" ]] || die "缺少 ${PARSER_FILE}"
}

check_remote_compose_config() {
  local file="${1:-${WORKER_COMPOSE}}"
  local remote_tmp="/tmp/minimax-m3-awq-compose-check-$$.yml"
  local remote_env="/tmp/minimax-m3-awq-cluster-check-$$.env"
