...
wait_dist_port() {
  ...
  log "等待 vLLM master-port ${host}:${port}（最多 ${MASTER_DIST_WAIT_SEC}s，就绪后立即启动 worker）..."
  while (( elapsed < MASTER_DIST_WAIT_SEC )); do
    if port_is_listening "${host}" "${port}" || port_is_listening "127.0.0.1" "${port}"; then
      log "master-port 已监听（${elapsed}s）→ 启动远程 worker"
      return 0
    fi
    if (( elapsed - last_log >= 15 )); then
      log "仍在等待 master-port... (${elapsed}s / ${MASTER_DIST_WAIT_SEC}s)"
      last_log=${elapsed}
    fi
    sleep "${MASTER_DIST_POLL_SEC}"
    elapsed=$((elapsed + MASTER_DIST_POLL_SEC))
  done
  die "超时：${MASTER_DIST_ADDR} 未就绪，请检查本机日志: docker logs ${CONTAINER_LLM}"
}
