...
check_local_compose_cli() {
  ...
  compose version >/dev/null 2>&1 || die "本机未找到 docker-compose"
}
...
check_remote_compose_cli() {
  ...
  remote "command -v docker-compose >/dev/null 2>&1" \
    || die "远程未安装 docker-compose（${REMOTE_HOST}）"
...
check_remote_image() {
  ...
  remote "docker image inspect '${DOCKER_IMAGE}' >/dev/null 2>&1" \
    || die "远程缺少镜像 ${DOCKER_IMAGE}（${REMOTE_HOST}，请先 docker pull）"
}

check_remote_compose_config() {
  local file="${1:-${WORKER_COMPOSE}}"
  local remote_tmp="/tmp/minimax-m3-compose-check-$$.yml"
  local remote_env="/tmp/minimax-m3-cluster-check-$$.env"
...
ensure_env() {
  ...
  case "${scope}" in
    ...
    master)
      check_local_docker
      check_local_compose_cli
      check_local_compose_config "${COMPOSE_FILE}" "node-44"
      ensure_local_hosts
      ...
      ;;
    worker_file)
      check_local_compose_config "${WORKER_COMPOSE}" "node-43"
      ;;
    remote)
      check_remote_ssh
      check_remote_docker
