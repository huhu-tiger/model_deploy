...
def cosyvoice3_example():
    ...
    # zero_shot usage
    for i, j in enumerate(cosyvoice.inference_zero_shot('习近平总书记高度重视和关心体育事业，党的十八大以来多次寄语中国体育健儿。每次见到大家，总书记都会点赞运动员们的优异成绩和拼搏精神，勉励大家奋力拼搏、再创佳绩，为建设体育强国作出新贡献。', 'You are a helpful assistant.<|endofprompt|>希望你以后能够做的比我还好呦。',
                                                        './public/upload/t.wav', stream=False)):
        torchaudio.save('zero_shot_{}.wav'.format(i), j['tts_speech'], cosyvoice.sample_rate)
