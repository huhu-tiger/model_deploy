...
```json
curl -X POST http://127.0.0.1:6003/v1/download-and-upload \
-H "Content-Type: application/json" \
-d '{
    "download_url_jsonpath": [
        "$.data[*].audio_url",
        "$.data[*].video_url",
        "$.data[*].image_url",
    ...
    "data": [
        {
            "status": "complete",
            "title": "宁静钢琴冥想",
            "audio_url": "minio://<bucket>/56cb7d08-604b-41ad-932c-3a0fab5db506.mp3",
            "image_url": "minio://<bucket>/image_56cb7d08-604b-41ad-932c-3a0fab5db506.jpeg",
            "image_large_url": "minio://<bucket>/image_large_56cb7d08-604b-41ad-932c-3a0fab5db506.jpeg",
            "avatar_image_url": "minio://<bucket>/sAura12.jpg"
        }
    ]
}
```
...
2. 使用 `curl` 测试接口：
   ```bash
   curl -X POST \
        http://127.0.0.1:6002/v1/download-and-upload \
        -H "Content-Type: application/json" \
        -d @request.json
   ```
   其中，`request.json` 是包含请求示例的文件。

3. 验证响应：
   - 确保返回的 URL 是 MinIO 的下载地址。
   - 验证文件是否成功上传到 MinIO。